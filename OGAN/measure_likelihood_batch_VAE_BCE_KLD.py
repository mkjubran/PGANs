...
if __name__ == "__main__":
 ...
 for j in range(0, args.number_samples_likelihood, args.OLbatchSize):
    ...
    likelihood_samples = engine_OGAN.get_likelihood_VAE_BCE_KLD(args,device,netE,optimizerE,sample_G1,netDec,args.save_likelihood_folder, logvar_first)
    likelihood_sample = torch.mean(likelihood_samples,0)
    likelihood_Dec_train.append(likelihood_samples.tolist())
    ...
    if (j+args.OLbatchSize <= samples_G1test.shape[0]):
         ...
         likelihood_samples = engine_OGAN.get_likelihood_VAE_BCE_KLD(args,device,netE,optimizerE,sample_G1,netDec,args.save_likelihood_folder,logvar_first)
         likelihood_sample = torch.mean(likelihood_samples,0)
         likelihood_Dec_test.append(likelihood_samples.tolist())
